...
def run_correlate_rearr(curr_img, mode_img, ux_tmp, ux, uxx_tmp, uxx, uxy_tmp, uxy, uy_tmp, uy, uyy_tmp, uyy, size1, size2, width, height):
...
def run_mode_rearr_y(mode_img, uy_tmp, uy, uyy_tmp, uyy, size1, size2, width, height):
    ...
    correlate1d_y(rearr, np_weights, width, height, uyy)
...
def update_corr_rearr_y(curr_img, mode_img, ux_tmp, ux, uxx_tmp, uxx, uxy_tmp, uxy, size1, size2, width, height):
    ...
    correlate1d_y(rearr, np_weights, width, height, ux)
    ...
    correlate1d_y(rearr, np_weights, width, height, uxx)
    ...
    correlate1d_y(rearr, np_weights, width, height, uxy)
...
def vid_runner(vidcap, mode_img, weights, data_range):
    ...
    ux_t, uy_t, uxx_t, uyy_t, uxy_t = setup(992, 660)
    ...
    while cont and frame_count < 50:
        ...
        #run_correlate(masked_curr_img, masked_mode_noblur_img, ux_tmp, ux_t, uxx_tmp, uxx_t, uxy_tmp, uxy_t, uy_tmp, uy_t,
        #              uyy_tmp, uyy_t, width, height)

        #tester(uxy, cm_scipy)

        S = run_math(cov_norm, data_range, ux, uy, uxx, uyy, uxy)
        #S = run_math(cov_norm, data_range, ux_t, uy_t, uxx_t, uyy_t, uxy_t)
        #S = S.transpose()
        diff_s = normalize_diff(S, width, height)
...
"""
    ...
    print(ndi.correlate1d(im1, weights, axis=1))

    correlate1d_y_r(im1, np_weights, width, height, ux)
    ux = ux.transpose()
    print("rearr", ux)

    correlate1d_y(im1, weights, width, height, ux)
    print("normal", ux)
    print(weights)
"""
"""
